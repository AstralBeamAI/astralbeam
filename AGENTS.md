...
- Lint with Oxlint and format with oxfmt through each project's `lint` and `format` tasks and its own `.oxlintrc.json` or `oxlint.config.ts` and `.oxfmtrc.json`. Do not use `deno lint`, `deno fmt`, or Deno `lint`/`fmt` configuration.
