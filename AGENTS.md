...
- `examples/todos-rails` is a Ruby on Rails consumer outside the four Deno projects and their Oxlint, oxfmt, and Knip rules. Follow its `AGENTS.md` and validate it with its own `bin/ci`.
...
- Keep the four `oxlint.config.ts` files structurally identical, differing only in the presets for libraries the project uses and its generated-path ignores. Import presets with their rules and options, raise every severity to `error`, and give overlapping rules one owner.
...
- Keep the root `deno.jsonc` a launcher for the four projects, with `format` and `format:check` tasks and a root `.oxfmtrc.json` limited to files directly in the root. CI must run the root `format:check` task. Do not add a `workspace` field, project-specific lint configuration, or root copies of per-project tasks such as `check`, `test`, and `ready`.
