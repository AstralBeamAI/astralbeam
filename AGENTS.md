...
- Use Deno from the affected project directory (`platform`, `www`, `sdk`, `cli`, `examples/todos`, or `examples/linearity-react`) with `deno task <script>`, or from the repository root with `deno task --cwd <project> <script>`. The projects do not form a package-manager workspace. Deno is the only supported repository JavaScript runtime and package manager. Vite and npm tooling run through its compatibility layer. SDK consumer examples may use the host application's package manager.
- `examples/todos-rails` is a Ruby on Rails consumer outside the six Deno projects and their Oxlint, oxfmt, and Knip rules. Follow its `AGENTS.md` and validate it with its own `bin/ci`.
...
- Root `tsconfig.base.json` holds only the compiler options all six projects share. Each `tsconfig.json` `extends` it and keeps its own `jsx`, `lib`, `types`, `paths`, and file globs.
...
- Keep the six `oxlint.config.ts` files structurally identical, differing only in the presets for libraries the project uses and its generated-path ignores. Import presets with their rules and options, raise every severity to `error`, and give overlapping rules one owner.
...
- Keep the root `deno.jsonc` a launcher for the six projects, with `format` and `format:check` tasks and a root `.oxfmtrc.json` limited to files directly in the root. CI must run the root `format:check` task. Do not add a `workspace` field, project-specific lint configuration, or root copies of per-project tasks such as `check`, `test`, and `ready`.
...
- Run JavaScript and TypeScript tests with Vitest through the project's Deno task, never `Deno.test` or `deno test`. The `platform`, todos, and Linearity browser suites use Playwright through their own `e2e` tasks, which stay out of `check`, `test`, `ready`, and CI.
