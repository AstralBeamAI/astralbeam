...
- Use PostgreSQL `uuid` primary and foreign keys with database-generated `uuidv7()` defaults, `citext` for email identity, and `timestamp with time zone` without forced precision for application instants; PostgreSQL 18 is the minimum supported server version. A row whose ID is also its public identifier keeps the `uuid` key and adds the public prefix at one repository boundary, so no table stores a prefixed text key.
