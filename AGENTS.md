...
- Bump the version in every PR. Keep `sdk/package.json`, `cli/package.json`, and `platform/package.json` at one version, and update every pinned `@astralbeam/sdk@<version>` URL (`git grep '@astralbeam/sdk@'`) to match. Below 1.0, bump the minor version for breaking changes and the patch version otherwise. If the current version has no `v<version>` tag yet, keep it unless the PR needs a larger bump. [`CONTRIBUTING.md`](CONTRIBUTING.md) still tells outside contributors to leave versions to the maintainers.
