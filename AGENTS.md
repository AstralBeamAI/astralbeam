...
- Reuse `apps/webapp/src/utils/environment.ts` from application and standalone-tool configuration instead of adding package-local loaders or environment files. Shell, CI, and deployment variables must take precedence over file values.

## Shared UI

- `apps/webapp/src/ui` owns the monorepo's only `components.json` and all shadcn-generated components, hooks, and utilities; applications import them from `@astralbeam/ui`.
- Add components from the repository root with `vp run @astralbeam/webapp#ui add <component>`.

## Database

- Keep PostgreSQL and Drizzle code in the server-only `apps/webapp/src/db` directory.
- Run database commands from the repository root with `vp run @astralbeam/webapp#db <command>`.
