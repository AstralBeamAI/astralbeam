...
- Run JavaScript and TypeScript tests with Vitest through the project's Deno task, never `Deno.test` or `deno test`. The todos browser suite uses Playwright through its `e2e` task.
...
- Keep SDK/API consumer docs focused on setup, usage, behavior, and troubleshooting. Keep implementation policy in `AGENTS.md`, contributor setup in `SETUP.md`, and system rationale in `ARCHITECTURE.md`. When shortening a README, preserve useful reference sections, options tables, examples, and workflows. Before removing important content, verify that a linked reference already covers it. Otherwise, retain it and shorten only repeated wording. Link to the owning explanation, but preserve copy-pasteable common commands in files that already listed them. Do not add new command sections elsewhere. Include the working directory or use `deno task --cwd <project> <task>`. When creating an `AGENTS.md`, add a sibling `CLAUDE.md` symlink to it.
