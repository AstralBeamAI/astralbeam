...
- `/configure` (`src/routes/configure`) is the operator surface for database-backed config. Require the actual database-backed rate limiter before rendering operator sign-in; treat `deno task --cwd webapp db migrate` as the authoritative all-migrations operation instead of reconciling Drizzle bookkeeping at runtime, and never apply migrations from the browser. Authenticate short, stateless sessions only with the first active `DATABASE_ENCRYPTION_KEY` value; never use database credentials. Require production HTTPS and same-origin mutations, and trust forwarded host/protocol only when ingress overwrites them and blocks direct origin access. Mask values until the operator explicitly reveals them, and derive the app gate from process-cached configuration validity rather than a persisted completion marker.
