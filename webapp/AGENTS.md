...
- `organization_configuration.openai_api_key` is each organization's own encrypted model key and the only place one is stored. `/api/v1/chat` streams on it, the deployment holds none, and `/configure` has no model provider group. Read presence in SQL rather than decrypting, and send a browser nothing of the key but its last four characters, derived from the one stored copy on the settings page's own read.
- In development, newly created Organizations copy `OPENAI_API_KEY` from the server environment loaded from `webapp/.env.local` into their encrypted settings. Never replace an existing Organization key or use this default in production. Development seeds include dogfood and fill only missing keys.
