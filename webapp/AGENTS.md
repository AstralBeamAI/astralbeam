...
- `src/db` contains the database schema and migrations
  - The server-only guarded `src/db/index.ts` entry point exports the Promise Drizzle handle and the native Effect database service; import both through `@/db`. Build every pool there with `createDatabasePool` and give each driver its own: the Effect SQL client cancels the running query and releases or destroys its client whenever a fiber is interrupted, so a pool shared with Better Auth let an aborted request poison session lookups with "Client was closed and is not queryable". Keep `pg` as the only PostgreSQL client library for both drivers, and keep Better Auth on the Promise handle with native Effect workflows behind the one `ManagedRuntime` bridge. Keep the Effect client on `PgClient.fromPool` rather than `PgClient.make` or `PgClient.layer`, whose build-time `SELECT 1` probe a `ManagedRuntime` caches as a permanently failed build fiber. Keep each pool's `error` listener, because an unhandled pool `error` event terminates the process; do not add `allowExitOnIdle`, a third pool, a global PostgreSQL type-parser table, `postgres`/postgres-js, or Nitro/signal lifecycle plumbing. The migration runner holds its advisory lock and per-migration transactions on separate clients from the Promise handle's pool.
