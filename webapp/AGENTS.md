...
- `organization_configuration.openai_api_key` is each organization's own encrypted model key and the only place one is stored. `/api/v1/chat` streams on it, the deployment holds none, and `/configure` has no model provider group. Read presence in SQL rather than decrypting, and never send the key to a browser.
