...
- `/configure` (`src/routes/configure`) is the operator surface for database-backed config. Render operator sign-in before database migrations; skip its database login limiter only when the limiter table is missing, and fail closed for every other store error. After sign-in, list pending Drizzle migrations and apply only the exact names and digests the operator approved. Authenticate short, stateless sessions only with the first active `DATABASE_ENCRYPTION_KEY` value; never use database credentials. Require production HTTPS and same-origin mutations, and trust forwarded host/protocol only when ingress overwrites them and blocks direct origin access. Mask values until the operator explicitly reveals them, and derive the app gate from process-cached configuration validity and migration state rather than a persisted completion marker.
