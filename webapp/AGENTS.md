...
- `src/db` contains the database schema and migrations
  ...
  - Define shared PostgreSQL columns with `caseInsensitiveText()`, `timestampWithTimeZone()`, `timestamps()`, `lockVersion()`, and `uuidV7PrimaryKey()` from `src/db/lib/columns.server.ts`; do not inline equivalent Drizzle builders.
