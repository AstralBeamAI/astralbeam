...
- `src/db` contains the database schema and migrations
  ...
  - Keep shared PostgreSQL column semantics in `src/db/lib/columns.server.ts`; reuse its case-insensitive text, timestamp-with-time-zone, audit timestamp, lock-version, and PostgreSQL UUIDv7 builders rather than inlining those definitions.
