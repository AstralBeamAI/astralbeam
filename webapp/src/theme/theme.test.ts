...
describe("theme document schema", () => {
  test("accepts an exhaustive custom theme", () => {
...
describe("theme color and CSS utilities", () => {
  ...
  test("generates deterministic light and dark CSS", () => {
    ...
    expect(css).toMatch(
      /^\/\* Generated by the webapp theme compiler\. Do not edit\. \*\//u,
    )
    expect(css).toContain(":root,\n.dark {\n  --radius: 0.75rem;\n}")
    expect(css).toContain(":root {\n  --background: #123456;")
    expect(css).not.toContain(".light")
    expect(css).toContain(".dark {\n  --background: #abcdef;")
    expect(css.match(/^ {2}--/gmu)).toHaveLength(themeTokenNames.length * 2 + 1)
    expect(css.endsWith("\n")).toBe(true)
  })
