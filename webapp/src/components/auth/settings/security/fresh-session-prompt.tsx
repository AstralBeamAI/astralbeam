      ...
      <FieldGroup className="gap-4">
        ...
        {auth.emailAndPassword?.enabled
          ...
          : (
            <Button
              onClick={() => {
                const redirectTo = `${auth.basePaths.settings}/${auth.viewPaths.settings.security}`
                auth.navigate({
                  to:
                    `${auth.basePaths.auth}/${auth.viewPaths.auth.signIn}?reauthenticate=true&redirectTo=${
