...
import { PencilSimple as Pencil, Plus, Trash as Trash2 } from "@phosphor-icons/react"
...
export function OrganizationRoles({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {roles.isLoading ? <Spinner /> : roles.data?.length
        ? (
          <Card>
                ...
                <TableBody>
                  {roles.data.map((role) => (
                    <OrganizationRoleRow
                      ...
                      onDelete={() => {
                        if (!globalThis.confirm(localization.deleteRoleDescription)) {
                          return
                        }
                        deleteRole.mutate({
                          roleId: role.id,
      ...
      <RoleDialog
        ...
        {...(editingRole == null ? {} : { role: editingRole })}
