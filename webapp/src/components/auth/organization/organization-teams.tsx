...
export function OrganizationTeams() {
  ...
  const teamLimitReached = teamPolicy.maximumTeams !== undefined &&
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        ...
        >
          ...
          <Button
            className="self-end"
            type="submit"
            disabled={createTeam.isPending || isCreatingFields || teamLimitReached}
          >
      ...
      {teams.isPending ? <Spinner /> : teams.data?.length
        ? (
          teams.data.map((team) => (
            <TeamCard
              key={team.id}
...
function TeamCard({
  ...
  const memberLimitReached = maximumMembersPerTeam !== undefined &&
      ...
      <CardContent className="flex flex-col gap-4 pt-6">
        ...
        >
          ...
          <Button
            ...
            variant="destructive"
            onClick={() => {
              if (!globalThis.confirm(localization.deleteTeam)) return
        ...
        <div className="flex items-end gap-2">
          ...
          <Button
            disabled={!userId || addMember.isPending || memberLimitReached}
            onClick={() =>
              userId &&
