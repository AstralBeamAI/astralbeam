...
| `generateAuthToken`              | `{ url: "/api/astralbeam/token" }` | Your AstralBeam auth token endpoint as `{ url, ...RequestInit }`, or a minting function |
...
- Self-hosted deployments must set `apiUrl` to their own origin; the default points at the hosted cloud, and AstralBeam auth tokens are bearer credentials that should only reach the deployment that issued the API key.
...
- `generateAuthToken` is the only AstralBeam auth token option; the request form's init reaches `fetch` as given. See [Authentication](./authentication.md).
