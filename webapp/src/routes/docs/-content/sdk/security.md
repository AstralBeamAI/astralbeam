...
- Your server mints short-lived AstralBeam auth tokens (60–600 s) from the API key; the browser never sees the key.
...
- A published file is served by a short-lived signed ticket bound to a digest of the exact published bytes; the ticket is the whole download authorization, which is what lets an `<img>` load it.
