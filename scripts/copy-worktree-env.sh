...
source_file="$source_root/platform/.env.local"
destination_file="$worktree_root/platform/.env.local"

if [ "$source_file" != "$destination_file" ] && [ -f "$source_file" ] && [ ! -e "$destination_file" ] && [ ! -L "$destination_file" ]; then
  cp -p "$source_file" "$destination_file"
fi

source_database_env_file="$source_root/platform/.env.development.local"
database_env_file="$worktree_root/platform/.env.development.local"

if [ -f "$source_database_env_file" ]; then
  cp -p "$source_database_env_file" "$database_env_file"
else
  source_database_env_file="$worktree_root/platform/.env.development"
fi
...
SHARP_IGNORE_GLOBAL_LIBVIPS=1 deno task --cwd platform db-reset
