...
- `createAstralBeamTokenRoute()` in `@astralbeam/sdk/server`: fetch-standard handler factory replacing the copy-pasted 503/500/`no-store` endpoint. Removed in 0.6.0 (revised after implementation): `createAstralBeamAuthToken` is the only way to mint a token.
