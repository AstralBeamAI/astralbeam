...
Use the server entry with the one-time `key_<organization>_<key>_abo_<key>_<secret>` value from the dashboard to mint the token without exposing the API key to browser code:
