...
- Model token identities with a required stable Tenant ID plus a stable tenant-local TenantUser ID; preserve omitted optional names and admin claims, and keep extra JSON tenant and tenant-user fields forward-compatible.
- Mint organization-issued tokens for the `astralbeam` audience with explicit feature scopes; do not duplicate tenant identity into the optional JWT subject.
