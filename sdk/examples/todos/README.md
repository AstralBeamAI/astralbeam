...
The chat talks to a real agent: the app points `endpoint` at the webapp's `/api/chat`, passes a todo-specific `systemPrompt`, and registers `list_todos`, `add_todo`, and `set_todo_completed` tools that execute against the app's own React state.
