...
The app uses plain CSS with no Tailwind or shadcn/ui. That is the point: the widget's Tailwind-based UI lives entirely inside its shadow root, and the only host UI in the conversation is the app's own `TodoCard`, projected into the chat as a slotted custom component with live app state and handlers.
