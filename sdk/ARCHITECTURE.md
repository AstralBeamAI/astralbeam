...
- Chat state runs on TanStack AI's `useChat`. There is no agent yet: `src/chat/conversation.ts` scripts a demo conversation replayed through the real `useChat` lifecycle via `@shadcn/helpers/tanstack-ai`, driving two tool calls that `src/chat/chat-widget.tsx` interprets — `render_widget` (host-defined widget) and `ask_questionnaire` (inline multi-step questionnaire answered as a user message).
