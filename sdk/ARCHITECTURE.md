...
- Internal layout mirrors the webapp's: hand-written chat components sit one per file in `src/components` next to the shadcn-generated `src/components/ui`, and `src/lib` holds shared helpers (`utils.ts`, including shadcn's `cn`), constants (`constants.ts`), shared types (`types.ts`), and the debug logger (`debug.ts`).
