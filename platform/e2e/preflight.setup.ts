...
import { e2eDatabaseUrl, mailboxUrl, platformUrl } from "./worktree.ts"
...
test("the platform and the mail sink are both answering", async ({ request }) => {
  expect((await request.get(`${platformUrl}/api/status`)).status()).toBe(200)
...
test("the server is running against the suite's freshly prepared database", async ({ request }) => {
  ...
  const response = await request.get(platformUrl)
  expect(
