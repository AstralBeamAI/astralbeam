...
import { mailboxSmtpPort, operatorKey, platformUrl } from "../../worktree.ts"
...
test("an operator configures the deployment and an owner runs the dashboard end to end", async ({
  ...
  await test.step("an operator configures an unconfigured deployment", async () => {
    ...
    expect(resetLink).toContain(platformUrl)
  ...
  await test.step("a visitor signs up and verifies their email address", async () => {
    ...
    expect(verificationLink).toContain(platformUrl)
  ...
  await test.step("the owner invites a member, which sends a real invitation email", async () => {
    ...
    expect(emailLink(invitation, /accept-invitation/)).toContain(`${platformUrl}/auth/`)
