...
- Dogfood onboarding in `/configure` initializes a new environment with an explicit invitation after application configuration is saved and valid. Saving configuration alone must not provision an owner or send email. Drizzle creates a verified user without a password or legal acceptance, so `emailVerified` does not indicate invitation delivery. Keep the pending recipient editable until delivery succeeds. Replacement transfers only the dogfood membership and preserves the organization and credential. Organization fields lock at creation and display current database values by UUID. Reuse resources only for pending retries. Better Auth owns reset tokens and credential creation, without the Admin plugin.
