...
const platformDirectory = join(dirname(fileURLToPath(import.meta.url)), "..", "..")
const migrationsDirectory = join(platformDirectory, "src", "db", "migrations")
...
export function loadSeedEnvironment(): void {
  for (const [key, value] of Object.entries(loadEnv("development", platformDirectory, ""))) {
...
export function resolveSeedDatabaseUrl(): { url: string; databaseName: string } {
  ...
  if (!url) throw new Error("DATABASE_URL is required; run this from the `platform` directory")
  let parsed: URL
  try {
    parsed = new URL(url)
  } catch {
