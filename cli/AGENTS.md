...
- Never accept an API key as a flag or print one unmasked. Read it from `ASTRALBEAM_API_KEY`, a hidden prompt, or stdin. Store directory bindings only in the user config file, written atomically with `0600` permissions, never in the bound directory. Send keys only to `https://` or loopback URLs, and print the resolved organization to stderr before every keyed command.
