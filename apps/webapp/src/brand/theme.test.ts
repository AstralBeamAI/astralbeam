...
import { palette, theme } from "@/brand"
import { generateThemeCss, resolveThemePalette, type ThemeMode } from "@/theme/theme"
  ...
  test("keeps the brand theme and palette browser-safe", () => {
    const browserImport = spawnSync(
      ...
      [
        ...
        "--input-type=module",
        "--eval",
        'const brand = await import("./index.ts"); if (!brand.theme || !brand.palette) process.exit(1)',
      ],
      {
        encoding: "utf8",
