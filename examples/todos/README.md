...
3. Create an organization API key. Copy its public `key_<organizationSlug>_<keySlug>` ID and one-time `abo_...` secret.
4. Copy `.env.example` to `.env`. Configure the API-key ID and secret on the server and set the browser-safe `VITE_ASTRALBEAM_AGENT_ID` shown on the agent page. Only the API-key value is confidential. The organization slug in the key and agent IDs must match.
