...
The chat talks to a real organization-owned agent: the app points `apiUrl` at the webapp's `/api`, serves the SDK's default `/api/astralbeam/token` token route (a handwritten handler over `createAstralBeamChatToken`), mints a short-lived JWT from an organization API key for a fixed demo tenant user, and registers `get_todos`, `create_todo`, `update_todo`, and `delete_todo` tools that execute against the app's own React state.
