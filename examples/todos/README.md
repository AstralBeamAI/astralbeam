...
Attachments need no wiring — the composer takes them by default when the agent's policy allows them — and the prompt asks the agent to turn an attached file or screenshot into todos through the same tools. `samples/tasks.csv` is here to drag onto the composer: a data file is described to the agent as a column profile and read through `read_attachment`, so the agent knows the shape before it reads a row, and with a sandbox on the agent it can compute over the real file.
